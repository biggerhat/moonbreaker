<template>
    <Head :title="props.culture ? 'Edit A Culture' : 'Add A Culture'" />

    <div class="mx-2 mx-auto">
        <Card class="mx-auto">
            <template #title> {{ props.culture ? 'Edit A Culture' : 'Add A Culture' }}</template>
            <template #content>
...
const submit = () => {
    router.post(props.culture ? route("admin.cultures.update", props.culture.id) : route("admin.cultures.store"),
        formInfo.value
    );
};

onMounted(() => {
    formInfo.value.name = props.culture?.name ?? null;
    formInfo.value.description = props.culture?.description ?? null;
});
</script>
